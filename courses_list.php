...
?>
<?php
// courses_list.php - Page to list courses with CRUD links
...
$sql = "SELECT co.CourseID, co.CourseTitle, co.Credits, d.DepartmentName
        FROM Courses co
        JOIN Departments d ON co.DepartmentID = d.DepartmentID
        ORDER BY co.CourseID DESC"; // Order by ID for latest first
...
<body class="dark">
    ...
    <div class="container mx-auto p-6">
        ...
        <div class="mb-6 text-center">
            <a href="courses_add.php" class="bg-blue-600 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded-lg transition duration-300 ease-in-out shadow-lg">
        ...
        <div class="dark-card rounded-xl p-6">
             <h2 class="text-xl font-semibold mb-4 text-blue-300">All Courses</h2>
                    ...
                    <table class="min-w-full rounded-lg overflow-hidden dark-table">
                        <thead class="bg-gray-700">
                            <tr>
                                ...
                                <th class="py-3 px-4 text-left text-sm font-semibold text-gray-300">Title</th>
                                <th class="py-3 px-4 text-left text-sm font-semibold text-gray-300">Credits</th>
                        ...
                        <tbody class="text-gray-200">
                            <?php foreach ($courses as $course): ?>
                                <tr class="border-b border-gray-700 hover:bg-gray-700">
                                    <td class="py-3 px-4"><?php echo htmlspecialchars($course['CourseID']); ?></td>
                                    <td class="py-3 px-4"><?php echo htmlspecialchars($course['CourseTitle']); ?></td>
                                    <td class="py-3 px-4"><?php echo htmlspecialchars($course['Credits']); ?></td>
                                    <td class="py-3 px-4"><?php echo htmlspecialchars($course['DepartmentName']); ?></td>
                                    <td class="py-3 px-4">
                                        <a href="courses_edit.php?id=<?php echo $course['CourseID']; ?>" class="action-link" title="Edit Course">
                                            <i class="fas fa-edit"></i> Edit
                                        </a>
                                        <a href="courses_delete.php?id=<?php echo $course['CourseID']; ?>" class="action-link delete-link" title="Delete Course" onclick="return confirm('Are you sure you want to delete this course?');">
            ...
            <?php else: ?>
                <p class="text-gray-400">No courses found in the database.</p>
            <?php endif; ?>
        </div>
