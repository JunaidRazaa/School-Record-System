...
?>
<?php
// departments_add.php - Page to add a new department
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $stmt = $conn->prepare($sql);
     ...
     if ($stmt === false) {
        ...
        if ($stmt->execute()) {
            $message = "<div class='bg-green-800 border border-green-600 text-green-200 px-4 py-3 rounded relative mb-4' role='alert'>New department added successfully!</div>";
...
<body class="dark">
    ...
    <div class="container mx-auto dark-card rounded-xl shadow-md p-8 max-w-md">
        ...
        <form action="departments_add.php" method="POST" class="dark-form">
            <div class="mb-4">
                <label for="department_name" class="block text-sm font-bold mb-2">Department Name:</label>
                <input type="text" name="department_name" id="department_name" class="shadow appearance-none rounded-lg w-full py-2 px-3 leading-tight focus:outline-none focus:shadow-outline" required>
            </div>
